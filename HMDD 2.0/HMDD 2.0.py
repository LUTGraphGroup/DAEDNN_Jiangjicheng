...
def DAE(x_train):
    ...
    decoded = Dense(878, activation='tanh')(decoded)
...
def load_data():
    ...
    A = np.loadtxt('association_matrix.txt')
    ...
    mm = np.repeat(MM, repeats=383, axis=0)
    dd = mb.repmat(DD, 495, 1)
    H = np.concatenate((dd, mm), axis=1)  # (294712,1162)
